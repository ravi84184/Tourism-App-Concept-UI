...
public class CurvedBottomNavigationView extends BottomNavigationView {
    ...
    public final int CURVE_CIRCLE_RADIUS = 80;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mFirstCurveStartPoint.set((mNavigationBarWidth/2)
                -(CURVE_CIRCLE_RADIUS+40)
                -(CURVE_CIRCLE_RADIUS/50),0);

        // the coordinates (x,y) of the end point after curve
        mFirstCurveEndPoint.set((mNavigationBarWidth/2),CURVE_CIRCLE_RADIUS+8+(CURVE_CIRCLE_RADIUS/8));

        //same for second curve
        mSecondCurveStartPoint = mFirstCurveEndPoint;

        mSecondCurveEndPoint.set((mNavigationBarWidth/2)+(CURVE_CIRCLE_RADIUS+40)+(CURVE_CIRCLE_RADIUS/50)
                ,0);
